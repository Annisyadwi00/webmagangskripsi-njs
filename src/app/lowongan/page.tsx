          ...
          <div className="relative z-10 max-h-[90vh] w-full max-w-3xl overflow-y-auto rounded-3xl bg-white p-6 shadow-2xl dark:bg-slate-900">
            <div className="mb-6 flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
              <div>
                ...
                <p className="mt-1 text-sm text-slate-500 dark:text-slate-400">
                  {selectedLowongan.company} •{' '}
                  {selectedLowongan.location || 'Menyesuaikan'}
                </p>
              </div>
            ...
            <div className="grid grid-cols-1 gap-4 md:grid-cols-3">
              ...
              <DetailItem
                label="Kategori"
                value={selectedLowongan.kategori || '-'}
              />
              <DetailItem
                label="Paid/Unpaid"
                value={selectedLowongan.isPaid ? 'Paid' : 'Unpaid'}
              />
              <DetailItem
                label="Berlaku Sampai"
                value={formatDate(selectedLowongan.valid_until)}
              />
            </div>
