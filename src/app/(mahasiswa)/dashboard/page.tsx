  ...
  return (
    <div className="min-h-screen bg-slate-50 dark:bg-slate-900 transition-colors duration-300 flex font-sans relative overflow-hidden">
      ...
      <main className="flex-1 flex flex-col h-screen overflow-y-auto">
        ...
        <header className="bg-white/80 dark:bg-slate-900/80 backdrop-blur-xl shadow-sm border-b z-999 border-gray-200 dark:border-slate-800 px-6 md:px-10 py-5 md:py-6 flex justify-between items-center sticky top-0 z-10 transition-colors duration-300">
                  ...
                  <div className="flex items-center min-w-max px-4">
                    {statusProgress.map((step, idx) => (
                      <div key={idx} className="flex items-center relative">
                        <div className={`w-10 h-10 md:w-12 md:h-12 rounded-full flex items-center justify-center font-black text-base md:text-lg border-4 transition-colors ${step.done ? 'bg-emerald-500 border-emerald-200 dark:border-emerald-900 text-white' : step.active ? 'bg-[#1e3a8a] border-blue-200 dark:border-blue-900 text-white ring-4 ring-blue-500/20' : 'bg-gray-100 dark:bg-slate-700 border-white dark:border-slate-800 text-gray-400 dark:text-slate-500'}`}>
                          {step.done ? '✓' : idx + 1}
                        </div>
                ...
                <div className="inline-flex bg-gray-100 dark:bg-slate-800 p-1.5 rounded-2xl gap-1 overflow-x-auto max-w-full border border-gray-200 dark:border-slate-700">
                  {['Semua', 'Disetujui', 'Revisi', 'Pending'].map((status) => (
                    <button
                      ...
                      className={`px-5 py-2.5 rounded-xl text-xs font-black transition-all whitespace-nowrap ${filterLogbook === status
                ...
                {logbookFiltered.length === 0 ? (
                  ...
                  <div key={log.id} className="bg-white dark:bg-slate-800 rounded-[28px] border border-gray-200 dark:border-slate-700 shadow-sm hover:shadow-lg transition-all overflow-hidden flex flex-col md:flex-row group">
                    ...
                    <div className="md:w-48 bg-slate-50/50 dark:bg-slate-800/80 border-b md:border-b-0 md:border-r border-gray-100 dark:border-slate-700 p-6 flex flex-col items-center justify-center text-center relative overflow-hidden">
                      <span className={`px-4 py-1.5 rounded-full text-[10px] font-black uppercase tracking-widest mb-6 shadow-sm border z-10 ${log.status === 'Disetujui' ? 'bg-emerald-50 text-emerald-600 border-emerald-200 dark:bg-emerald-900/30 dark:border-emerald-800' :
