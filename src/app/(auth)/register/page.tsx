...
export default function RegisterPage() {
  ...
  const handleRegister = async (e: React.FormEvent) => {
    ...
    if (!passwordRegex.test(formData.password)) {
      setErrorMsg('Kata sandi tidak memenuhi standar keamanan minimum institusi.');
      return;
    }
    ...
    try {
      ...
      setSuccessMsg('Kredensial berhasil didaftarkan. Menginisialisasi proses verifikasi.');
  ...
  const handleVerifyOtp = async () => {
    ...
    // Simulasi penundaan jaringan untuk endpoint verifikasi
    setTimeout(() => {
  ...
  return (
    <div className="min-h-screen bg-gray-50 flex flex-col justify-center py-12 sm:px-6 lg:px-8 font-sans">
      <div className="sm:mx-auto sm:w-full sm:max-w-xl">
        <h2 className="mt-6 text-center text-3xl font-extrabold text-gray-900">Registrasi Kredensial</h2>
        <p className="mt-2 text-center text-sm text-gray-600">Otorisasi Akses Sistem Informasi Magang</p>
      </div>
      ...
      <div className="mt-8 sm:mx-auto sm:w-full sm:max-w-2xl">
        <div className="bg-white py-8 px-4 shadow-xl sm:rounded-3xl sm:px-10 border border-gray-100">
          ...
          {/* Kontrol Pemilihan Peran */}
          <div className="mb-8">
            <label className="block text-sm font-bold text-gray-700 mb-3">Kategori Otorisasi</label>
              ...
              {['Mahasiswa', 'Dosen', 'Admin'].map((role) => (
                <button
                  ...
                  className={`py-2 text-sm font-bold rounded-lg transition-all duration-200 ${
                    activeRole === role ? 'bg-[#1e3a8a] text-white shadow-md' : 'text-gray-500 hover:text-gray-700 hover:bg-gray-200'
                  }`}
                >
                  {role}
                </button>
          ...
          <form className="space-y-6" onSubmit={handleRegister}>
             <div>
              <label htmlFor="name" className="block text-sm font-bold text-gray-700 mb-1">Nama Identitas Resmi</label>
            ...
            <div>
              <label htmlFor="email" className="block text-sm font-bold text-gray-700 mb-1">Alamat Email Institusi</label>
            ...
            <div className="pt-2">
              <button type="submit" disabled={isLoading} className="w-full py-3.5 px-4 rounded-xl shadow-md text-sm font-bold text-white bg-[#1e3a8a] hover:bg-blue-900 transition-colors outline-none focus:ring-2 focus:ring-offset-2 focus:ring-[#1e3a8a] disabled:opacity-70 disabled:cursor-not-allowed">
                {isLoading ? 'Menjalankan Proses Eksekusi...' : 'Inisialisasi Pendaftaran'}
              </button>
            </div>
          </form>

          <div className="mt-6 text-center text-sm border-t pt-6 border-gray-100">
            <span className="text-gray-600">Kredensial sudah terdaftar dalam sistem? </span>
            <Link href="/login" className="font-bold text-[#1e3a8a] hover:text-blue-800 transition-colors">Akses Sistem Disini</Link>
          </div>
        </div>
      </div>

      {/* Komponen Modal Verifikasi OTP */}
      <AnimatePresence>
        {showOtpModal && (
